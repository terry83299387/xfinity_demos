...
$(function() {
	...
	$.ajax({
		...
		success : function(a, b, c) {
			...
			if(tmp == null){
				...
				var bas = tmp.split(',');
				document.getElementById('softs').innerHTML = bas[0];
				document.getElementById('pros').innerHTML = bas[1];
				document.getElementById('jobs').innerHTML = bas[2];
				if (bas[3] === 'null') {
					document.getElementById('jobTime').innerHTML = 0;
				} else {
					document.getElementById('jobTime').innerHTML = bas[3];
				}
			}
		}
	});
	...
	$.ajax({
		type : "post",
		url : "userStatisticByMonth.action",
		contentType : "application/x-www-form-urlencoded; charset=utf-8",
		data : {
			userCode : userCode,
			fromDate : fromDate,
...
function goToGroupStatistic() {
	var url = 'statistic/groupStatistic.jsp';
